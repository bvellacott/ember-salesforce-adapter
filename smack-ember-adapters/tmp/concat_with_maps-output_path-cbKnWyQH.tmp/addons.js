  ...
  var LSAdapter = _emberData['default'].Adapter.extend(_ember['default'].Evented, {
    ...
    findMany: function findMany(store, type, ids, opts) {
      ...
      for (var i = 0; i < ids.length; i++) {
        ...
        try {
          _smackEmberAdaptersAdaptersSmackHooks['default'].onFind(this.loadData(), type, res, record);
        } catch (e) {
    ...
    query: function query(store, type, _query /*recordArray*/) {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].onQuery(this.loadData(), type, results, allowRecursive);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("On find hook failed on: " + " type '" + type.modelName + "'.\n\n" + e));
      }
      var res;
...
define('smack-ember-adapters/adapters/smackHooks', ['exports', 'npm:smack-js-compiler'], function (exports, _npmSmackJsCompiler) {
	...
	var getPackSequence = function getPackSequence(packageRec, hash) {
		if (!hash['package']) hash['package'] = { records: {} };
		var records = hash['package'].records;
		var sequence = [];
		for (var cur = packageRec; !!cur; cur = records[cur.parent]) sequence.push(cur.name);
	...
	var beforeCreateHandlers = {
		...
		'package': function _package(packageRec, hash) {
			var packs = getPackSequence(packageRec, hash);
			createPackage(packs, namespace);
			serialiseNamespace();
		},
