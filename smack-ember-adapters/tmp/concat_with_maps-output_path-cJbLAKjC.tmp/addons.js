...
define('ember-data', ['ember-data/index', 'ember', 'exports'], function(__index__, __Ember__, __exports__) {
  'use strict';
  var keys = Object.keys || __Ember__['default'].keys;
  var forEach = Array.prototype.forEach && function(array, cb) {
    array.forEach(cb);
...
define('ember-inflector', ['ember-inflector/index', 'ember', 'exports'], function(__index__, __Ember__, __exports__) {
  'use strict';
  var keys = Object.keys || __Ember__['default'].keys;
  var forEach = Array.prototype.forEach && function(array, cb) {
    array.forEach(cb);
...
define('smack-ember-adapters/adapters/ls-adapter', ['exports', 'ember', 'ember-data', 'npm:where-clause-evaluate', 'smack-ember-adapters/adapters/smackHooks', 'smack-ember-adapters/adapters/storageForm'], function (exports, _ember, _emberData, _npmWhereClauseEvaluate, _smackEmberAdaptersAdaptersSmackHooks, _smackEmberAdaptersAdaptersStorageForm) {
  // TODO :
  ...
  var fromStorageForm = _smackEmberAdaptersAdaptersStorageForm['default'].fromStorageForm;
  var toStorageForm = _smackEmberAdaptersAdaptersStorageForm['default'].toStorageForm;
  ...
  var LSAdapter = _emberData['default'].Adapter.extend(_ember['default'].Evented, {
    ...
    findRecord: function findRecord(store, type, id, opts) {
      ...
      try {
        var record = _ember['default'].A(fromStorageForm(store, type, namespace.records[id], this.loadData()));
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error(e));
      }
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].onFind(this.loadData(), type, record, allowRecursive);
      } catch (e) {
    ...
    findMany: function findMany(store, type, ids, opts) {
      ...
      for (var i = 0; i < ids.length; i++) {
        ...
        try {
          _smackEmberAdaptersAdaptersSmackHooks['default'].onFind(hash, type, res, record);
        } catch (e) {
    ...
    query: function query(store, type, _query /*recordArray*/) {
      ...
      try {
        var records = {};
        for (var id in namespace.records) records[id] = fromStorageForm(store, type, namespace.records[id], hash);
        results = this._query(records, _query);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error(e));
      }
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].onQuery(hash, type, results, allowRecursive);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("On find hook failed on: " + " type '" + type.modelName + "'.\n\n" + e));
      }
      var res;
    ...
    findAll: function findAll(store, type) {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].onFindAll(this.loadData(), type, namespace.records);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("On find hook failed on: " + " type '" + type.modelName + "'.\n\n" + e));
      }
    ...
    createRecord: function createRecord(store, type, snapshot) {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].beforeCreate(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("Before create hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
      try {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].afterCreate(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("After create hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
      return _ember['default'].RSVP.resolve(_ember['default'].A(recordHash));
    },
    ...
    updateRecord: function updateRecord(store, type, snapshot) {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].beforeUpdate(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("Before update hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
      try {
      ...
      this.persistData(type, namespaceRecords, hash);
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].afterUpdate(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("After update hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
      return _ember['default'].RSVP.resolve(_ember['default'].A(recordHash));
    },
    ...
    deleteRecord: function deleteRecord(store, type, snapshot) {
      ...
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].beforeDelete(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("Before delete hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
      delete namespaceRecords.records[id];
      this.persistData(type, namespaceRecords, hash);
      try {
        _smackEmberAdaptersAdaptersSmackHooks['default'].afterDelete(hash, type, recordHash);
      } catch (e) {
        return _ember['default'].RSVP.reject(new Error("After delete hook failed on: " + " type '" + type.modelName + "' for the id '" + recordHash.id + "'.\n\n" + e));
      }
    ...
    persistData: function persistData(type, data, hash) {
    ...
    _namespaceForType: function _namespaceForType(type, storage) {
...
define('smack-ember-adapters/adapters/smackHooks', ['exports', 'npm:smack-js-compiler'], function (exports, _npmSmackJsCompiler) {
	...
	var compile = _npmSmackJsCompiler['default'].compile;
	var getPackage = _npmSmackJsCompiler['default'].getPackage;
	var createPackage = _npmSmackJsCompiler['default'].createPackage;
	var removePackage = _npmSmackJsCompiler['default'].removePackage;
	...
	var removePackagesAndFunctions = function removePackagesAndFunctions(pack, hash) {
		var cur = pack;
		var packs = hash['package'].records;
		var childPacks = [];
		for (var id in packs) if (packs[id].parent === pack.id) childPacks.push(packs[id]);pack;
		childPacks.forEach(function (c) {
			removePackagesAndFunctions(c, hash);
		});
		if (hash['compilation-unit'] && hash['compilation-unit'].records) {
			var units = hash['compilation-unit'].records;
			var childUnits = [];
			for (var id in units) if (units[id].pack === pack.id) childUnits.push(units[id]);
			childUnits.forEach(function (u) {
				delete units[u.id];
			});
		}
...
define('smack-ember-adapters/adapters/storageForm', ['exports', 'ember', 'smack-ember-adapters/adapters/ls-adapter'], function (exports, _ember, _smackEmberAdaptersAdaptersLsAdapter) {
  'use strict';

  var form = {
    toStorageForm: function toStorageForm(store, type, record) {
      if (!record) return record;
      var storeForm = _ember['default'].copy(record);
      type.eachRelationship(function (name, desc) {
        if (desc.kind !== 'hasMany') return;
        var inverseDesc = type.inverseFor(name, store);
        if (inverseDesc && inverseDesc.name && inverseDesc.name !== '' && inverseDesc.kind !== 'hasMany') storeForm[name] = inverseDesc.name;
    ...
    fromStorageForm: function fromStorageForm(store, type, storeForm, hash) {
      if (!storeForm || !hash[type.modelName] || !hash[type.modelName].records) return storeForm;
      var record = _ember['default'].copy(storeForm);
      type.eachRelationship(function (name, desc) {
        if (desc.kind !== 'hasMany') return;
        var inverseDesc = type.inverseFor(name, store);
        var ids = [];
...
define('smack-ember-adapters/models/compilation-unit', ['exports', 'ember-data/model', 'ember-data/attr', 'ember-data/relationships'], function (exports, _emberDataModel, _emberDataAttr, _emberDataRelationships) {
	...
	exports['default'] = _emberDataModel['default'].extend({
		...
		pack: (0, _emberDataRelationships.belongsTo)('package', { inverse: 'units' }),
...
define('smack-ember-adapters/models/package', ['exports', 'ember-data/model', 'ember-data/attr', 'ember-data/relationships'], function (exports, _emberDataModel, _emberDataAttr, _emberDataRelationships) {
	'use strict';

	exports['default'] = _emberDataModel['default'].extend({
		name: (0, _emberDataAttr['default'])('string'),
		parent: (0, _emberDataRelationships.belongsTo)('package', { inverse: 'children' }),
		children: (0, _emberDataRelationships.hasMany)('package', { inverse: 'parent' }),
		units: (0, _emberDataRelationships.hasMany)('compilation-unit', { inverse: 'pack' })
	});
});
...
define('smack-ember-adapters/serializers/ls-serializer', ['exports', 'ember', 'ember-data'], function (exports, _ember, _emberData) {
  'use strict';
...
define('smack-ember-adapters', ['smack-ember-adapters/index', 'ember', 'exports'], function(__index__, __Ember__, __exports__) {
  'use strict';
  var keys = Object.keys || __Ember__['default'].keys;
  var forEach = Array.prototype.forEach && function(array, cb) {
    array.forEach(cb);
